...
namespace ErrorHandler
{
	...
	DWORD WINAPI CleanUpThread()
	{
		...
		DriverRequest::SendProcessIDs(0);
		Sleep(400);
		exit(1);
	}
	...
	bool LoadDriver()
	{
		return load_driver("C:\\Unfriendly-V2-Driver.sys", "Unfriendly Driver", "Unfriendly-Anti Cheat");
	}
	bool UnloadDriver()
	{
		return delete_service("Unfriendly Driver");
	}
	void ErrorMessage(std::string ErrorCode, int ErrorType)
	{
		CloseHandle(NamedPipe::pipe);
		std::string ErrorMessage = "Unfriendly-Scan\n\nUnfriendly Anti Cheat Failure (Contact Support )\nError Code:  " + ErrorCode;
