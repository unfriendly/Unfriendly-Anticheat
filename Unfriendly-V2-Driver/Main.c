...
VOID DriverUnload(PDRIVER_OBJECT pDriverObject)
{
	...
	UNICODE_STRING UnfriendlySymbolName;
	RtlInitUnicodeString(&UnfriendlySymbolName, L"\\DosDevices\\UnfriendlyDriver"); // Giving the driver a symbol
	if (ObHandle)
	{
	...
	IoDeleteSymbolicLink(&UnfriendlySymbolName);
...
NTSTATUS DriverEntry(PDRIVER_OBJECT pDriverObject, PUNICODE_STRING pUniStr)
{
	...
	UNICODE_STRING UnfriendlyDriverName, UnfriendlySymbolName;
	NTSTATUS NtRet = STATUS_SUCCESS;
	PDEVICE_OBJECT pDeviceObj;
	RtlInitUnicodeString(&UnfriendlyDriverName, L"\\Device\\UnfriendlyDriver"); // Giving the driver a name
	RtlInitUnicodeString(&UnfriendlySymbolName, L"\\DosDevices\\UnfriendlyDriver"); // Giving the driver a symbol
	UNICODE_STRING deviceNameUnicodeString, deviceSymLinkUnicodeString;
	NTSTATUS NtRet2 = IoCreateDevice(pDriverObject, 0, &UnfriendlyDriverName, FILE_DEVICE_UNKNOWN, FILE_DEVICE_SECURE_OPEN, FALSE, &pDeviceObj);

	IoCreateSymbolicLink(&UnfriendlySymbolName, &UnfriendlyDriverName);
